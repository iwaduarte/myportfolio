...
const Contact = () => {
  ...
  const handleSubmit = evt => {
    ...
    axios
      .post('https://email.iwaduarte.dev/send-email', {
        from,
        subject,
        message,
        appSource
  ...
  if (!allowSend && typeof allowSend === 'boolean') {
    return (
      <div className="flex-container direction-column space-evenly m-auto profile-form">
    ...
    <>
      <form className="flex-container direction-column space-evenly m-auto profile-form">
        ...
        <div className="subject   ">
          ...
          <input
            onChange={evt => handleChange(evt, 'setSubject')}
            value={subject}
            className="home-input-field"
            type="text"
          />
        ...
        <div className="body mt-1 mb-0 ">
          ...
          <textarea
            onChange={evt => handleChange(evt, 'setMessage')}
            value={message}
            className="home-input-field textarea mt-1 mb-0"
          />
        </div>
        <button onClick={handleSubmit} className="videos-button mt-1" type="submit ">
          {' '}
          Submit
        </button>
      </form>
