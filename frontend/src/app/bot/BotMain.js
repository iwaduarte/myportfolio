...
const BotMain = props => {
    ...
    const update = (props) => {
        ...
        let emitUpdate = _elapsedTimeInAPeriod + refreshTime >= COUNT_DOWN_MILLISECONDS
            && _elapsedTimeInAPeriod - refreshTime < COUNT_DOWN_MILLISECONDS;
        //setting next question
        emitUpdate && setBotState(prevState => {

            if (prevState.timeout.length) { //array is populated


                if (typeof prevState.timeout[0] === 'string')
                    return botStatesEN[prevState.timeout[0]] || prevState;
    ...
    return <>
        <div className="bot-main container flex-container">
            ...
            {image
                ? <>
                    {image.message &&
                    <h1 className="inline-block txt-align-center mt-0">{image.message}</h1>}
