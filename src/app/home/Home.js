...
import quoteAPIList from "../bot/QuoteAPIList";
...
const Home = props => {
    ...
    const loadVideo = (action) => {
        if (action === 'prev')
        return setVideoYT( prevState => prevState.index !== 0 ? {video: videosList[prevState.index - 1], index: prevState.index-1} : prevState);
        if (action === 'next')
            return setVideoYT( prevState => videosList[prevState.index + 1] ? {video: videosList[prevState.index + 1], index: prevState.index+1}: prevState);
